        ...
        for category in os.listdir(data_dir):
            ...
            for img_path in os.listdir(img_dir):
                try:
                    print(img_path)
                    # try:
                    fd = ','.join([str(i) for i in get_image_features_by_path(os.path.join(img_dir, img_path))])
                    fd += ',' + str(label) + '\n'
                    data_file.write(fd)
...
def get_image_features_by_path(image_path):
    image = cv2.imread(image_path)
    return get_image_features(image)
